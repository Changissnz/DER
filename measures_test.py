...
def test__angle_between_two_points_clockwise():
    ...
    assert angle_between_two_points_clockwise(p,p1) == 0.0, "wrong angle for 0.0"
    assert angle_between_two_points_clockwise(p,p2) == 90.0, "wrong angle for 00.0"
    assert angle_between_two_points_clockwise(p,p3) == 180.0, "wrong angle for 180.0"
    assert angle_between_two_points_clockwise(p,p4) == 270.0, "wrong angle for 270.0"
    return -1

'''
uniform weights
'''
def test__similarity_measure__type43___uw_():
    ...
    v1 = np.copy(bounds[:,0])
    v2 = np.copy(bounds[:,1])
    sm = similarity_measure__type43(v1,v2, bounds, weights)
    assert sm == 1.0, "invalid 1"

    # test for commutativity
    sm = similarity_measure__type43(v2,v1, bounds, weights)
...
def test__similarity_measure__type43___nuw_():
    ...
    weights = np.asarray(np.ones(4),dtype='float')
    sm1 = similarity_measure__type43(v1,v2,bounds,weights)

    print("SIM MEASURE ", sm, sm1)

    assert sm < sm1, "wrong relation for measure"

def test__similarity_measure__type44___uw_():
    ...
    v2 = np.copy(bounds[:,1])
    sm = similarity_measure__type44(v1,v2,bounds,weights)
    ##print("SM ",sm)

    # case: `basically` at right endpoint
    v1 = np.copy(bounds[:,0])
    v2 = np.array([60.0,-69.5,60.0,25.0])
    sm1 = similarity_measure__type44(v1,v2,bounds,weights)
    ##print("SM ",sm1)
    assert sm1 < sm, "[0] invalid sim. measure"

    sm1_ = similarity_measure__type43(v1,v2,bounds,weights[:,0])
    ...
    sm2 = similarity_measure__type44(v1,v2,bounds,weights)
    ##print("SM ",sm2)

    # case: v2 is at 1/2
    v2 = np.copy(bounds[:,1]) - (0.5 * diff)
    sm3 = similarity_measure__type44(v1,v2,bounds,weights)
    ##print("SM ",sm3)
    ...
    sm3_ = similarity_measure__type44(v1_,v2_,bounds,weights)
    ##print("@SM ", sm3_)
    sm3__ = similarity_measure__type44(v2_,v1_,bounds,weights)
    ##print("@SM ",sm3_)
    assert sm3_ == sm3__, "[3] measure must be commutative"

    # case: v2 is at 3/8
    v2 = np.copy(bounds[:,0]) + (3.0/8.0 * diff)
    sm4 = similarity_measure__type44(v1,v2,bounds,weights)
    ##print("SM ",sm4)
    assert sm3 < sm4

    # case: v2 is at 2^(-17)
    v2 = np.copy(bounds[:,0]) + (1.0/131072.0 * diff)
    sm5 = similarity_measure__type44(v1,v2,bounds,weights)
    assert sm5 == 0.5, "incorrect sim. measure"

    v1 = np.copy(bounds[:,1])
    v2 = np.copy(bounds[:,1]) - (1.0/131072.0 * diff)
    sm6 = similarity_measure__type44(v1,v2,bounds,weights)
    assert sm6 == 1.0 + (1.0 - sm5), ""

def test__similarity_measure__type44___nuw_():
...
def test___equal_matrices():
    m = np.arange(16).reshape((4,4))
    n = np.arange(16).reshape((4,4))
    assert equal_iterables(m,n), "basic equality test failed"

def test_intersection_of_bounds___1():
    ...
    b2 = np.array(b2)
    intersection = intersection_of_bounds(b1,b2)
    ...
    assert equal_iterables(intersection,trueIntersection), "bounds intersection incorrect"
    ...
    intersection = intersection_of_bounds(b1,b3)
