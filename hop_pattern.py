...
class HopPattern:
    ...
    def value_at(self):
        # case: single
            ...
            # r -> l
        """
        if type(self.headIndex) == type(None):
            head = 0 if self.hopDirection[0] >= 0 else 1
        else:
            head = (self.headIndex + 1) % 2
        """

        #print(head, " PTRVAL ", self.ptrValue1)
        head = 0 if self.hopDirection[0] >= 0 else 1
    ...
    def hop_one(self, rev = False):
        ...
        if self.headIndex == None:
            ...
            ##head = 1 if q[0] >= 0.0 else 1
        else:
            head = self.headIndex
        ##print("BEF ",self.ptrValue1)
